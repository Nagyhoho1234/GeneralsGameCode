...
#include <dinput.h>
#else
// Portable equivalent (native port plan Phase 1): this file only uses
// DirectInput's DIK_* scan code values (standard PC keyboard scan set 1,
// plus 0x80 for extended keys), not any DirectInput COM API, so the
// values are reproduced directly rather than gating out real
// functionality. These match the public DirectX SDK's dinput.h.
