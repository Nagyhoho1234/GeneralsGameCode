...
namespace
{
	...
	{
		fprintf(stderr, "RENDERCAMERATRANSFORM_FAIL: %s\n", what);
		g_AnyFailure = true;
	}
	...
	const char* const kArchiveName = "RenderCameraTransform.big";
...
int main()
{
	...
	// function/lambda) per dx8wrapper.h:711's "friend int main();" grant.
	{
		...
		{
			fprintf(stderr, "RENDERCAMERATRANSFORM_FAIL: init round-trip failed, aborting\n");
			g_AnyFailure = true;
			return 1;
		}
		...
		if (!robj)
		{
			fprintf(stderr, "RENDERCAMERATRANSFORM_FAIL: could not create render object, aborting remaining checks\n");
		...
		REF_PTR_RELEASE(scene);
		Check(scene == nullptr, "8a. REF_PTR_RELEASE(scene) nulled the pointer");

		delete TheTerrainLogic;
		TheTerrainLogic = nullptr;
		Check(TheTerrainLogic == nullptr, "8b. TerrainLogic::~TerrainLogic() ran (reset()'s own real body: deleteWaypoints/deleteBridges/PolygonTrigger::deleteTriggers, all safe with empty lists)");

		delete TheFramePacer;
		TheFramePacer = nullptr;
		Check(TheFramePacer == nullptr, "8c. FramePacer::~FramePacer() ran (real body: timeEndPeriod(1))");

		delete TheWritableGlobalData;
		Check(TheWritableGlobalData == nullptr, "8d. GlobalData::~GlobalData() nulled TheWritableGlobalData");

		WW3DErrorType shutdown_result = WW3D::Shutdown();
		Check(shutdown_result == WW3D_ERROR_OK, "8e. WW3D::Shutdown returned WW3D_ERROR_OK");
		Check(!WW3D::Is_Initted(), "8f. WW3D::Is_Initted() is false after Shutdown");

		delete TheW3DFileSystem;
		TheW3DFileSystem = nullptr;
		Check(_TheFileFactory == nullptr, "8g. ~W3DFileSystem restored _TheFileFactory");
		...
		Check(TheFileSystem == nullptr && TheLocalFileSystem == nullptr && TheArchiveFileSystem == nullptr && TheW3DFileSystem == nullptr,
			"8h. all four file-system singletons torn down and nulled");
	...
	if (g_AnyFailure)
	{
		fprintf(stderr, "RENDERCAMERATRANSFORM_FAIL: one or more checks failed (see above)\n");
		return 1;
	}

	printf("RENDERCAMERATRANSFORM_OK: all checks passed\n");
	return 0;
}
