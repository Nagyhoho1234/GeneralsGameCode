...
// DX8Wrapper::Do_Onetime_Device_Dependent_Inits/_Shutdowns,
// Set_Default_Global_Render_States (and its F2DW helper), and
// Invalidate_Cached_Render_States moved verbatim to dx8wrapper_draw.cpp
// (native port plan Phase 5(a) Milestone 6, Draft 26 Steps 2+4 combined) -
// see that file's comment for why, and dx8wrapper_gl.cpp for the GL
// backend's Compute_Caps this closure now calls instead of the device-
// querying DX8Caps constructor.
...
// DX8_Assert moved verbatim to dx8wrapper_draw.cpp (native port plan
// Phase 5(a) Milestone 3, finding 1) - referenced from the draw-path
// functions that moved there too.
...
// DX8Wrapper's high-level draw-path functions - Set_Vertex_Buffer (both
// overloads), Set_Index_Buffer (both overloads), Draw_Sorting_IB_VB, Draw,
// Draw_Triangles (both overloads), Draw_Strip, Apply_Render_State_Changes,
// and Set_Viewport - moved verbatim to dx8wrapper_draw.cpp (native port
// plan Phase 5(a) Milestone 3, finding 1). They touch the device only
// through DX8CALL/device vtable calls PortableD3D8 already declares.
...
// DX8Wrapper::Set_Light(unsigned, const D3DLIGHT8*) moved verbatim to
// dx8wrapper_draw.cpp (native port plan Phase 5(a) Milestone 3, finding 1).
// Set_Light(unsigned, const LightClass&) stays here - it drags light.cpp.
...
// Set_Light_Environment moved to dx8wrapper_draw.cpp (native port plan
// Phase 5(a) Milestone 5, Draft 24 Step 2) - see that file's comment for
// why.
...
// DX8Wrapper's name-decoder family (Get_DX8_Render_State_Name, Get_DX8_Texture_Stage_State_Name,
// Get_DX8_*_Value_Name, and the Get_DX8_*_Name family) moved verbatim to
// dx8wrapper_draw.cpp (native port plan Phase 5(a) Milestone 3, finding 1) -
// pure string/switch tables with zero D3D calls, referenced from the
// MESH_RENDER_SNAPSHOT_ENABLED blocks inside Apply_Render_State_Changes,
