...
void ShaderClass::Apply()
{
	...
	if (diff & pri_mask) {
		...
		if ( voodoo3 && (PricArg2==D3DTA_DIFFUSE) &&
			...
			// NOTE (unify #555): Generals has a WW3D::Is_Coloring_Enabled() TFACTOR override here that
			// Zero Hour's voodoo3-stage-shuffling rewrite never ported over - upstream left it as
			// `#if 0`'d dead code referencing cArg2/aArg2/cOp/aOp, variable names that don't even exist
