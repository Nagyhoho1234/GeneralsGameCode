...
// Set_Light_Environment, moved verbatim out of dx8wrapper_d3d8.cpp (native
// port plan Phase 5(a) Milestone 5, Draft 24 Step 2, finding 5):
// dx8renderer.cpp (soon-to-be-portable, Draft 24 Step 5) calls this, and it
// used to live only in the Windows-only backend file - a guaranteed POSIX
// link failure the moment the mesh pipeline links. Its body is pure
// LightEnvironmentClass reads (all inline in lightenvironment.h - no link
// dependency on lightenvironment.cpp itself) + Set_Light(unsigned, const
// D3DLIGHT8*) calls (already above, in this same portable file).
// Set_Light(unsigned, const LightClass&) deliberately stays behind in
// dx8wrapper_d3d8.cpp - it still drags light.cpp, still uncalled by this
// closure.
